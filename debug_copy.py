    ...
    if rl_enabled:
        ...
        if learn:
            ...
            while env.simulator.is_running:
                ...
                if len(env.simulator.jobs_manager.waiting_queue) >= 6:
                    i = 0
                    while i <= 64:
                        if i == 9 or i == 10 or i == 11 or i == 21 or i == 22:
                            print('debug')
                        duped_env = deepcopy(env)
                        next_observation, reward, done = duped_env.step(T.tensor([i/64], dtype=T.float))
                        observation = next_observation
                        workshop[i] = reward
                        i+=1
                    df = pd.DataFrame(list(workshop.items()), columns=['num_nodes', 'reward'])
                    df.to_csv('output.csv', index=False)
                features_, mask_ = observation
                features_ = features_.to(device)
                action, logprob = get_action(model, features_)
                next_observation, reward, done = env.step(T.tensor([1], dtype=T.float))
                observation = next_observation
